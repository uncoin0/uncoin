...
uint256 hashGenesisBlock("0x00000118632ebfce267c7940d14552fa837b1c044b0ebab32374c810463d6962");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UNCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "UNCoin Signed Message:\n";
...
{
    // UNCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1)
		return 720000000 * COIN;

    nSubsidy >>= (nHeight / 356400000);
...
{
    // UNCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "UNCoin X13 Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xed31ac717f0ebf5c5d7979aa6a30e35223146e72a2c7bc4f10ed74e77568c2ef"));
...
unsigned char pchMessageStart[4] = { 0xed, 0xce, 0xa9, 0xae }; // UNCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UNCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UNCoinMiner : ProcessBlock, block not accepted");
    }
